...
impl<T> RenderSources<T> {
    ...
    {
        RenderSources {
            pair: [generate(RenderDir::Forward), generate(RenderDir::Reverse)],
        }
    }

    pub fn src(&self, dir: RenderDir) -> &T {
        match dir {
    ...
    pub fn dst(&self, dir: RenderDir) -> &T {
        match dir {
...
impl<T> RenderMotion<T> {
    ...
    pub fn get(&self, dir: RenderDir) -> &T {
        match dir {
            RenderDir::Forward => &self.pair[0],
            RenderDir::Reverse => &self.pair[1],
        }
